...
class Book {
  ...
  String get details =>
      "$titre de $auteur, publié en $date, $page pages, Genre: $genre";
}
...
class Emprunt {
  ...
  String get info =>
      "$nom a emprunté ${book.titre} le ${DateTime.now().toString().split(":${DateTime.now().second}")[0]}h et a jusqu'à le ${retour.toString().split(" ")[0]} pour le rendre";
...
class Library {
  ...
  void listBooks() => books
      .forEach((i) => print("Livre n°${books.indexOf(i) + 1} : ${i.details}"));

  void listEmprunts() => emprunts.forEach(
      (j) => print("Emprunt n°${emprunts.indexOf(j) + 1} : ${j.info}"));

  void listLateReturns() => emprunts.where((e) => e.isLate).forEach((e) => print(
      "Emprunt en retard n°${emprunts.indexOf(e)} : ${e.nom} est en retard sur son emprunt du livre : ${e.book.titre}"));
}

void main() {
  var livre1 =
      Book("Harry Potter", "J. K. Rowling", Genre.Fantastique, 1997, 320);
